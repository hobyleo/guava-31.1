...
abstract class SmoothRateLimiter extends RateLimiter {
   ...
   * 下一次可以获取令牌的起始时间
   * 由于RateLimiter允许预消费，上次请求预消费令牌后，下次请求需要等待到nextFreeTicketMicros后才能获取令牌
   */
  ...
  final long reserveEarliestAvailable(int requiredPermits, long nowMicros) {
    resync(nowMicros); // 根据当前时间更新桶中的令牌数量
    long returnValue = nextFreeTicketMicros; // 上次计算的nextFreeTicketMicros
    double storedPermitsToSpend = min(requiredPermits, this.storedPermits); // 可以直接消费的令牌数
    double freshPermits = requiredPermits - storedPermitsToSpend; // 还需要等待获取的令牌数
    long waitMicros =
        storedPermitsToWaitTime(this.storedPermits, storedPermitsToSpend)
            + (long) (freshPermits * stableIntervalMicros); // 根据freshPermits计算需要等待的时间

    this.nextFreeTicketMicros = LongMath.saturatedAdd(nextFreeTicketMicros, waitMicros); // 更新下次可以获取令牌的时间
    this.storedPermits -= storedPermitsToSpend; // 更新当前存储令牌数
    return returnValue;
  }
   ...
   * 此函数会在每次获取令牌之前调用，逻辑为：
   * 如果当前时间晚于nextFreeTicketMicros，则计算该段时间内应该生成多少令牌
   */
